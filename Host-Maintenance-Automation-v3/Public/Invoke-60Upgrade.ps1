...
.EXAMPLE
Invoke-Upgrade -vCenter vcenter.contoso.com -HostImport "c:\Temp\Hosts.txt"
#>
function Invoke-Upgrade {
    ...
    Begin {
		...
		$FromAddress = "PSScripts@ehi.com"
		$smtpServer = "smtp.corp.erac.com" #this shouldnt change
        ...
        Try {
            Foreach  ($CurrentHost in $HostList) {
                ...
                #Check ILO Version
                Update-ILO -HostParam $CurrentHost -logs $logFile -ilocred $Credential  

                #Remove the Conflicting VIBs#
                Remove-60VIBS -HostParam $CurrentHost -logs $logFile
                
				Write-ToLogFile -logPath $logfile "The VIBS have been removed."
				
                #Get the UpgradeBasline#
                $baselines = Get-Baseline -Entity $CurrentHost -Inherit
                
				Write-ToLogFile -logPath $logfile "The $CurrentHost will be upgraded with $baselines.Name."
                ...
                If ($ComplianceStatus.Status -eq 'NotCompliant') {
					...
					Update-60Hosts -HostParam $CurrentHost -BaselineInfo $baselines
                ...
                #Checking Host Compliance - Patch if not Compliant
                $ComplianceStatus = Get-Compliance -Entity $CurrentHost
                If ($ComplianceStatus.Status -eq 'NotCompliant') {
				...
				#Register PowerPath#
                #Set-PowerPath -HostParam $CurrentHost -logs $logFile -domain $domain -usernames $user -passwords $pass 
			...
			Write-ToLogFile -logPath $logfile " - script section executed" -fore gray
        }#End Try
